class UpdateStoreCoordinates < ActiveRecord::Migration[8.0]
  def up
    # Update all stores with accurate Google Maps coordinates (verified December 2025)
    Store.find_by(name: 'Kompally')&.update(
      latitude: 17.5366834,
      longitude: 78.4902905
    )

    Store.find_by(name: 'Loft')&.update(
      latitude: 17.4323211,
      longitude: 78.3825729
    )

    Store.find_by(name: 'Film Nagar')&.update(
      latitude: 17.4141907,
      longitude: 78.407981
    )

    Store.find_by(name: 'Jubilee Hills')&.update(
      latitude: 17.4277035,
      longitude: 78.4051957
    )

    Store.find_by(name: 'Kokapet')&.update(
      latitude: 17.38648,
      longitude: 78.3373395
    )
  end

  def down
    # No need to revert - coordinate updates are data changes
  end
end
